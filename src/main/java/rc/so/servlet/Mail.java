...
public class Mail extends HttpServlet {
                ...
                if (iduser.equals("---")) {
                    usr.forEach(user -> {
                        if (EmailValidator.getInstance().isValid(user.getEmail())) {
                            fadmail(pr, iduser, dataoggi, st,
                                    user.getNome().toUpperCase() + " " + user.getCognome().toUpperCase(), datainvito,
                                    user.getEmail());
                            out.print("success");
                            out.flush();
                        } else {
                    ...
                    if (usr.stream().anyMatch(us -> us.getIdallievi().equals(iduser))) {
                        Optional<GenericUser> optionalUser = usr.stream().filter(us -> us.getIdallievi().equals(iduser))
                                .findAny();
                        if (optionalUser.isPresent()) {
                            GenericUser user = optionalUser.get();
                            if (EmailValidator.getInstance().isValid(user.getEmail())) {
                                boolean es = fadmail(pr, iduser, dataoggi, st,
                                        user.getNome().toUpperCase() + " " + user.getCognome().toUpperCase(),
                                        datainvito, user.getEmail());
                                if (es) {
                                    out.print("success");
                                } else {
    ...
    public static boolean fadmail(String idprogetti_formativi, String idsoggetto,
        ...
        try {
            ...
            String sql4 = "SELECT user,psw FROM fad_access WHERE type='S' AND idprogetti_formativi = ? AND idsoggetto = ? AND data = ? AND room = ?";
            DbSSO dbs = new DbSSO();
            ...
            try (PreparedStatement ps4 = db1.getC().prepareStatement(sql4)) {
                ...
                try (ResultSet rs4 = ps4.executeQuery()) {
                    if (rs4.next()) {
                        ...
                        String upd = "UPDATE fad_access SET psw = ? WHERE type='S' AND idsoggetto = ? AND data = ?";
                        try (PreparedStatement ps5 = db1.getC().prepareStatement(upd)) {
                            ps5.setString(1, md5psw);
                            ps5.setString(2, idsoggetto);
